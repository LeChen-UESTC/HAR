...
可用模板变量：`project_root`、`active_split`、`dataset_name`、`dataset_num_classes`、`split_name`、`text_num_classes`、`description_variant`、`description_variant_short`、`text_variant`、`text_mode`、`projector_type`、`projector_mode`、`embedding_model`、`text_pooling`、`main_label_alpha`。
