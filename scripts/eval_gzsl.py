...
def run(ctx: dict, args) -> None:
    ...
    if checkpoint:
        ...
        logger.info(
            "Loaded checkpoint: path=%s epoch=%s saved_gzsl_h_mean=%s saved_gzsl_unseen_top1=%s",
            checkpoint,
            payload.get("epoch"),
            saved_metrics.get("gzsl_h_mean"),
            saved_metrics.get("gzsl_unseen_top1"),
        )
    else:
...
def main() -> None:
    args = parse_common_args("Evaluate skeleton embedding model under GZSL candidates.")
